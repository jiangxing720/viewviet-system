...
返回 JSON（不加代码块）：
{
  ...
  try {
    if (!openai) {
      res.status(503).json({ error: "AI 功能未配置，请检查 AI_INTEGRATIONS_OPENAI_API_KEY" });
      return;
    }
...
router.post("/admin/generate-card-image", async (req, res): Promise<void> => {
  ...
  try {
    if (!openai) {
      res.status(503).json({ error: "AI 图片功能未配置，请检查 AI_INTEGRATIONS_OPENAI_API_KEY" });
      return;
    }
    ...
    res.json({ imageUrl: imageUrl ?? `data:image/png;base64,${b64}` });
  } catch (e: any) {
    res.status(500).json({ error: e?.message ?? String(e) });
  }
});
