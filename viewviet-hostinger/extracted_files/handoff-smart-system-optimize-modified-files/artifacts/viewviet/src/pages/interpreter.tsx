...
import {
  Mic, Volume2, VolumeX, Trash2, RotateCw, AlertCircle, FileText, Loader2, StopCircle
} from "lucide-react";
...
export default function Interpreter() {
  ...
  const {
    ...
    permissionError, start, stop, clearLog, replay
  } = useInterpreter(langA, langB, "both", false, autoSpeak);
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col md:flex-row overflow-hidden font-sans touch-none select-none">
      ...
      <div className="absolute top-3 left-1/2 -translate-x-1/2 z-40 rounded-full bg-black/60 px-4 py-2 text-xs font-medium text-white backdrop-blur">
        {running ? (status === "translating" ? "实时翻译中" : status === "speaking" ? "正在播报译文" : "实时同声传译已开启") : "点击麦克风开始实时同声传译"}
      </div>
