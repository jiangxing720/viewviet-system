...
export default function AdminLegal() {
  ...
  const fillFormFromImportedArticle = (data: any) => {
    form.reset({
      title: data.title ?? "",
  ...
  const handleImportUrl = async () => {
    ...
    try {
      ...
      fillFormFromImportedArticle(data);
      setImportUrl("");
  ...
  const handleImportText = async () => {
    ...
    setIsImporting(true);
    try {
      const res = await fetch(`${BASE}/api/admin/legal-articles/import-text`, {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
      ...
      fillFormFromImportedArticle(data);
      setImportText("");
      toast({ title: "AI 已根据复制内容生成文章，请检查后保存" });
    } catch (e: any) {
      toast({ title: e?.message ?? "提取失败", variant: "destructive" });
    } finally { setIsImporting(false); }
  };
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl space-y-6">
      ...
      <div className="rounded-xl border border-dashed border-primary/40 bg-primary/5 p-4">
        ...
        <div className="mt-3 space-y-2">
          <Label className="text-xs">公众号复制全文导入</Label>
          <Textarea
            rows={5}
            placeholder="如果公众号链接抓取失败，把公众号正文全选复制到这里，然后点击一键生成。"
            value={importText}
          ...
          <div className="flex items-center justify-between gap-3">
            <p className="text-xs text-muted-foreground">{importText.trim().length} 字符</p>
            <Button variant="outline" size="sm" onClick={handleImportText} disabled={isImporting || importText.trim().length < 80} className="gap-1.5">
              {isImporting ? <><Loader2 className="w-4 h-4 animate-spin" />生成中...</> : <><Sparkles className="w-4 h-4" />复制内容一键生成</>}
            </Button>
          </div>
        </div>
        {isImporting && <p className="text-xs text-muted-foreground mt-2">正在调用 AI 提取全文、摘要、分类和国家，约 15-30 秒...</p>}
      </div>
