...
export default function Lawyers() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2" data-testid="text-lawyers-title">Legal Professionals</h1>
        <p className="text-muted-foreground">Verified cross-border legal experts for Chinese and Vietnamese communities</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8">
        <div className="relative flex-1">
          ...
          <Input className="pl-9" placeholder="Search by name..." value={search} onChange={(e) => setSearch(e.target.value)} data-testid="input-search-lawyers" />
        </div>
        <select
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {lawyers.map((lawyer: any) => (
            <Card key={lawyer.id} className="hover:shadow-lg transition-all duration-300 hover:-translate-y-1" data-testid={`card-lawyer-${lawyer.id}`}>
              <CardContent className="p-5 space-y-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  {lawyer.isFeatured && <Badge className="ml-auto flex-shrink-0 text-xs">Featured</Badge>}
                </div>
                ...
                <div className="flex gap-2 pt-2 border-t">
                  {lawyer.email && (
                    <a href={`mailto:${lawyer.email}`} data-testid={`button-email-${lawyer.id}`}>
                      <Button variant="ghost" size="icon" className="w-8 h-8"><Mail className="w-4 h-4" /></Button>
                    </a>
                  )}
                  {lawyer.phone && (
                    <a href={`tel:${lawyer.phone}`} data-testid={`button-phone-${lawyer.id}`}>
                      <Button variant="ghost" size="icon" className="w-8 h-8"><Phone className="w-4 h-4" /></Button>
                    </a>
                  )}
                  {lawyer.whatsapp && (
                    <a href={`https://wa.me/${lawyer.whatsapp.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" data-testid={`button-whatsapp-${lawyer.id}`}>
                      <Button variant="ghost" size="icon" className="w-8 h-8"><MessageSquare className="w-4 h-4" /></Button>
                    </a>
                  )}
                  <Button asChild size="sm" className="ml-auto" data-testid={`button-contact-${lawyer.id}`}>
